...
import { useCreateOrderMutation } from "@/redux/features/orders/ordersApi"; // Import RTK mutation
...
const CourseDetails = ({ data, setRoute, setOpen: openAuthModal }: Props) => {
  ...
  // Use createOrder mutation from RTK Query
  ...
  const handleOrder = async () => {
    ...
    try {
      ...
      if (response?.success) {
        toast(response.message || "Order placed successfully!");
        refetch(); // Refresh user data
      } else {
        toast("Order failed: " + (response?.message || "Order placed successfully!"))
        
      }
    } catch (err: any) {
      console.error("❌ Order Error:", err);
      toast("Error processing order: " + (err?.data?.message || "Unknown error"));
    }
  };
              ...
              <div className="flex items-center">
                {isPurchased ? (
                  ...
                  <div
                    ...
                    onClick={handleOrder}
                  >
                    {isLoading ? "Processing..." : `Buy Now ${data.price}$`}
                  </div>
                )}
              </div>
